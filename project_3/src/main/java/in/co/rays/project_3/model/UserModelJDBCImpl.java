...
public class UserModelJDBCImpl implements UserModelInt {
	...
	public UserDTO findByLogin(String login) throws ApplicationException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			ps = conn.prepareStatement("select * from ST_USER where LOGIN=?");
	 ...
	 * connn = null; List list = new ArrayList(); try {
	 * 
	 * connn = JDBCDataSource.getConnection(); PreparedStatement pstmt =
	 * connn.prepareStatement(sql.toString()); pstmt.setLong(1, dto.getRoleId());
